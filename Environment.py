    ...
    def update_state(self, action, bandwidth, waitTime, task, t):
        ...
        for edge in self.cecGraph.edges:
            if edge in list1:
                ...
                # path_bandwidthһ����0�����ԲŻ���waitTime˵���������ͬʱͨ��������waitTime�Ƿ���Ч��
        ...
        if max_edge_waitTime <= waitTime and waitTime > 0:
            # ��������ȴ�һ��ʱ���ٽ��еĻ������defer execution��ʹ��path_bandwidth��Ԥ���и���, ��Χ��[pathbandwidth, future_pathbandwidth]
            max_edge_waitTime = waitTime
            if path_bandwidth > bandwidth:
                path_bandwidth = bandwidth
        else:
            if path_bandwidth > bandwidth and bandwidth > 0:
                path_bandwidth = bandwidth
        begin_t = int(t+max_edge_waitTime)
        ...
        #     p_waitTime = p_waitTime + waitTimeList[0]
        #     next_waitTime = self.state_df[device_name][self.state_df['time'] == end_ctime+p_waitTime].values.tolist()[0]
        #     while(next_waitTime != 0):
        #         p_waitTime = p_waitTime + next_waitTime
        #         next_waitTime = self.state_df[device_name][self.state_df['time'] == end_ctime+p_waitTime].values.tolist()[0]
        #假设一个Device上最多运行N=2个任务
        N = 2
        # N = 20
        waitTimeList = self.state_df[device_name][self.state_df['time'] == end_ctime].values.tolist()
        p_waitTime = p_waitTime + waitTimeList[0]

        if target_action_device.taskNum < N:
            print("reduce waiting time:", p_waitTime)
            p_waitTime = 0
            task_num = target_action_device.taskNum+1
            target_action_device.setTaskNum(task_num)
        else:
            if p_waitTime == 0:
                target_action_device.setTaskNum(0)
            else:
                # print("time:", end_ctime+p_waitTime)
                # print(self.state_df)
                next_waitTime = self.state_df[device_name][self.state_df['time'] == end_ctime+p_waitTime].values.tolist()[0]
                while(next_waitTime != 0):
                    p_waitTime = p_waitTime + next_waitTime
        ...
        # print("end_ctime", end_ctime)
        # print("p_waitTime", p_waitTime)
        ...
        for i in range(begin_ptime, begin_ptime + int(ptime)+1):
            if i in self.state_df['time'].values.tolist():
                ...
                # resource
                if target_action_device.taskNum <= N:
                    temp_val = self.state_df.loc[self.state_df['time'] == i, device_name].item()
                    self.state_df.loc[self.state_df['time'] == i, device_name] = max(temp_val, begin_ptime + int(ptime)+1-i)
                else:
