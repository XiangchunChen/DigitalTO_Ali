...
def estimate_time(task, deviceList, device_time_dic, cecGraph):
    # if random.random() < 0.1:
    #     i = 2
    #     computation_time = float(task.cload)/deviceList[i-1].cpuNum
    #     waiting_time = 0
    #     if i in device_time_dic.keys():
    #         waiting_time = device_time_dic[i]
    #     # TODO
    #     bandwidth = 2
    #     for edge in cecGraph.edges:
    #         bandwidth = cecGraph[edge[0]][edge[1]]['weight']
    #         break
    #     # print("bandwidth", bandwidth)
    #     communication_time = float(task.dataSize)/bandwidth
    #     temp_time = computation_time + waiting_time + communication_time
    #     return i, temp_time
    ...
    #     return i, temp_time
    greedy_index = 1
    greedy_time = sys.maxsize
    # 不超过 1 hop的情况下
    one_hop_index = 3
    for i in range(1, one_hop_index):
        computation_time = float(task.cload)/deviceList[i-1].cpuNum
        waiting_time = 0
        if i in device_time_dic.keys():
            waiting_time = device_time_dic[i]
        ...
        communication_time = float(task.dataSize)/bandwidth
        temp_time = computation_time + waiting_time + communication_time
        if temp_time < greedy_time:
            greedy_time = temp_time
            greedy_index = i
    print("task.source, action", task.source, greedy_index)
    return greedy_index, greedy_time
        ...
        while not episode_done or time_step < max_time:
            ...
            if len(tasks) != 0:
                for i in range(len(tasks)):
                    ...
                    M = 3 # M是一个参数，表示一个设备同时保存M个task
                    target_device_index, greedy_time = estimate_time(task, env.deviceList, device_time_dic, env.cecGraph)
                    # if env.deviceList[target_device_index-1].taskNum >= 2:
                    if target_device_index in device_time_dic.keys():
                        device_time_dic[target_device_index] = device_time_dic[target_device_index] + greedy_time
                    else:
                        device_time_dic[target_device_index] = greedy_time
                    if env.deviceList[target_device_index-1].taskNum > M:
                        env.deviceList[target_device_index-1].setTaskNum(0)
                    # else:
                    #     num = env.deviceList[target_device_index-1].taskNum+1
                    #     env.deviceList[target_device_index-1].setTaskNum(num)

                    target_bandwidth = -1 #默认带宽
                    target_waitTime = -1
